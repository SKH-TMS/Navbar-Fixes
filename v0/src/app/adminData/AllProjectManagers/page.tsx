...
// Define User Type
interface User {
  UserId: string; // Include UserId if available and potentially useful
  firstname: string;
  lastname: string;
  email: string;
  profilepic: string;
...
export default function AllProjectManagers() {
  ...
  const [selectedUsers, setSelectedUsers] = useState<string[]>([]); // Store emails for selection
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401 || response.status === 403) {
            ...
            router.push("/userData/LoginUser"); // Adjust login path
          }
  ...
  // Handle checkbox selection (remains the same)
  ...
  // Handle delete function (remains the same)
  ...
  // Handle update function (remains the same)
  const handleUpdate = () => {
    if (selectedUsers.length === 0) {
  ...
  // Navigate to Project Manager Details Page (remains the same)
