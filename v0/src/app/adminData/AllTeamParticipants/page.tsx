...
export default function AllTeamParticipants() {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // Redirect only on specific errors like 401/403 if desired
          if (response.status === 401 || response.status === 403) {
            toast.error("Unauthorized access. Redirecting to login.");
            router.push("/userData/LoginUser"); // Adjust login path if needed
          }
  ...
  const handleDelete = async () => {
    ...
    // --- IMPORTANT: Use the correct delete endpoint ---
    // Assuming you have an endpoint specifically for deleting team participants
    // or a general user delete endpoint that handles roles.
    // Adjust the endpoint and payload as necessary.
    // If deleting a participant requires special logic (e.g., removing from teams),
    // ensure the backend handles it. For now, using a generic deleteUsers endpoint.
    const deleteEndpoint = "../../api/adminData/deleteUsers"; // Verify this endpoint
