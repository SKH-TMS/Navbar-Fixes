...
export default function ProfileAdmin() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) {
      ...
      toast.error("Failed to fetch admin data. Please try again later."); // Redirect Admins to AdminLogin
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success("Picture Uploaded sucessfully");
      } else {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <div className="mt-4 p-4 border rounded-lg shadow-md w-80 text-center">
          ...
          <button
            ...
            className="mt-4 px-4 py-2 bg-rose-300 text-white rounded"
          >
            Show All ProjectManagers
          </button>
          {/* Show All Team Participants Button */}
          <button
            onClick={() => router.push("AllTeamParticipants")}
            className="mt-4 px-4 py-2 bg-cyan-300 text-white rounded"
          >
            Show All team Participants
          </button>
        </div>
      </div>
    </div>
